...
export function VehiclesManager() {
  ...
  return (
    <div className="space-y-6">
        ...
        <Modal isOpen={showForm} onClose={() => setShowForm(false)} title="Nuevo Vehículo">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vehicles.map(v => (
                <div key={v.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative group hover:shadow-md transition-all">
                    <button onClick={() => handleDelete(v.id)} className="absolute top-4 right-4 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-2 bg-white/80 rounded-full">
                        <Trash2 size={18} />
                    </button>
